...
- clock domains: clk
- reset domains: rst_x, rst_y

#### [HIGH] score=75 RDC: `src_reg` -> `dst_reg` (module `rdc_example`)

- source domain: `clk=clk;rst=rst_x` @ rdc_example.sv:25:13
- destination domain: `clk=clk;rst=rst_y` @ rdc_example.sv:33:13
- width: 1
- synchronizer evidence: no_synchronizer_evidence
- HEURISTIC finding
- rationale:
  - register 'dst_reg' (rst=rst_y) reads register 'src_reg' (rst=rst_x) on the same clock 'clk' across asynchronous reset domains
